require 'fog/core/collection'
require 'fog/compute/models/bt/key_pair'

module Fog
  module BT
    class Compute

      class KeyPairs < Fog::Collection

        model Fog::BT::Compute::KeyPair

        # Used to create a key pair.  There are 3 arguments and only name is required.  You can generate a new key_pair as follows:
        # BT.key_pairs.create(:name => "test", :fingerprint => "123", :private_key => '234234')
        #
        # ==== Returns
        #
        #<Fog::BT::Compute::KeyPair
        #  name="test",
        #  fingerprint="3a:d3:e5:17:e5:e7:f7:de:fe:db:1b:c2:55:7d:94:0b:07:2e:05:aa",
        #  private_key="-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
        #>
        #
        # The key_pair can be retreived by running BT.key_pairs.get("test").  See get method below.
        #

        def initialize(attributes)
          super
        end

        # Returns an array of all key pairs that have been created
        #
        # BT.key_pairs.all
        #
        # ==== Returns
        #
        # <Fog::BT::Compute::KeyPairs
        #  key_name=nil
        #  [
        #    <Fog::BT::Compute::KeyPair
        #      name="test",
        #      fingerprint="1f:26:3d:83:e7:4f:48:74:c3:1c:e6:b3:c7:f6:ec:d8:cb:09:b3:7f",
        #      private_key=nil
        #    >
        #  ]
        #>
        #

        def all(*key_names)
          data = connection.describe_key_pairs(*key_names).body
          load(data['keySet'])
        end

        # Used to retreive a key pair that was created with the BT.key_pairs.create method.
        # The name is required to get the associated key_pair information.
        #
        # You can run the following command to get the details:
        # BT.key_pairs.get("test")
        #
        # ==== Returns
        #
        #>> BT.key_pairs.get("test")
        #  <Fog::BT::Compute::KeyPair
        #    name="test",
        #    fingerprint="1f:26:3d:83:e7:4f:48:74:c3:1c:e6:b3:c7:f6:ec:d8:cb:09:b3:7f",
        #    private_key=nil
        #  >
        #

        def get(key_name)
          if key_name
            self.class.new(:connection => connection).all(key_name).first
          end
        end

      end

    end
  end
end

